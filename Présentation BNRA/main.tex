...
\begin{frame}{Signature BNRA}
	...
	\pause
	\textbf{\small Messages received with the same value come from the same process.}
\end{frame}
...
\begin{frame}{Well quasi-orders}
		...
		\onslide<1-12>{\onslide<2->{$(4,3)$}\onslide<4>{$\to (4,2)$}\onslide<5->{\hspace{-1.35cm}$\to (2,4)$}\onslide<6->{$\to (7,1)$}\onslide<7->{$\to (0,5)$}\onslide<8->{$\to (8,0)$}\onslide<9->{$\to (3,1)$}\onslide<10->{$\to (1,2)$}\onslide<11->{$\to (0,0)$}}

...
\begin{proof}
	...
	For every $i$, let $\atrans_i$ the $i$-th transition in $\localrun$, and let $\theta_i : \nset{1}{r} \to V_{in} \cup \set{\bot}$ be the function such that 
	\[
	\begin{cases}
		& 	\theta_i(j) = v \in V_{in} \text{ if } $v$ \text{ is the value of the $j$th register after $i$ steps of $u$}\\ 
		& 	\theta_i(j) = \bot \text{ if the value of the $j$th register after $i$ steps of $u$ is not in $V_{in}$}
	\end{cases}
	\]
	...
	There are $|\transitions|^{2M}$ possible sequences for $s_i$ and $[(\size{V_{in}}+1)^r]^{2M+1}$ possible sequences for $\Theta_i$.
	By the pigeonhole principle there exist two indices $i_a, i_b$ such that the sequences $s_{i_a}$ and $s_{i_b}$ are equal (as $|\transitions|, \size{V_{in}} \leq n$). 
	There exist two infix "local runs" $\localrun_a: (q_1, \localdata_1) \step{*} (q_2, \localdata_2)$, $\localrun_b: (q_3, \localdata_3) \step{*} (q_4, \localdata_4)$ in $\localrun$ such that $(q_2,\localdata_2)$ appears strictly before $(q_3,\localdata_3)$ in $\localrun$ and $\localrun_a$ and $\localrun_b$ both have the same sequences of transitions, which we call $s$, and of values from $V_{in}$, which we call $\Theta$.
	...
	We build a "trace" $\atrace$ such that $(q_1,\localdata_1) \step{\atrace} (q_4, \localdata_4)$ where the underlying sequence of transitions of $\atrace$ is $s$ and the locations of values of $V_{in}$ matches $\Theta$.
	...
	\begin{itemize}
		...
		\item if $\aval_a \in V_{in}$ then $\aval_a = \theta_j(i) = \aval_b$, we set $\aval = \aval_a$.
		\item if $i \in I$ and $\aval_a \notin V_{in}$ then $\theta_j(i) = \bot$ and thus $\aval_b \notin V_{in}$, then 
		\begin{itemize}
	...
	If $\localdata_a(i) \in V_{in}$ then $\localdata(i) = \theta(i) = \localdata_a(i) = \localdata_b(i)$.
	
	The only problematic cases are when $i \in I$, $\localdata_a(i) \notin V_{in}$ and either $\anact = \quotemarks{\diseqtestact}$ or $\anact = \quotemarks{\eqtestact}$. 

...
\begin{restatable}{lemma}{lemShortLocalRuns}
	...
	There exists a primitive recursive function $\towerfun(n)(r)$ such that, for every protocol $\prot$ with $r$ registers per process, for every "local run" $\localrun: (q, \localdata) \step{*} (q', \localdata')$ in $\prot$, for every $V_{in}, V \subseteq \nats$ finite such that $V$ contains every message value appearing in $\localrun$, there exists a "local run" $\localrun': (q, \localdata) \step{*} (q', \localdata')$ such that $\length{\localrun'} \leq \towerfun(\size{\prot} + \size{V_{in}})(r) -1$ and:
	\begin{enumerate}
...
\begin{proof}
	...
	We define the function $\towerfun(n)(k)$ recursively as $\towerfun(n)(0) = n+1$ and $\towerfun(n)(k+1) = 2(\towerfun(n)(k) +1) (n^2)^{2(\towerfun(n)(k)+1)} +2$. This function is clearly primitive recursive, although non-elementary. A straightforward induction allows one to bound (roughly) $\towerfun(n)(k)$ between the towers of exponentials
	...
	Suppose that the property is true for any protocol with $\leq k$ "active registers", and consider a run $\localrun: (q_i,\localdata_i) \step{*} (q_f,\localdata_f)$ with $k+1$ "active registers" such that $\length{\localrun} > \towerfun(n)(k+1)-1$.
	...
	Let $I \subseteq \nset{1}{r}$ the set of "active registers" in $\localrun$, $|I| = k+1$. Let $M:= \towerfun(n)(k){+}1$, we have $\towerfun(n)(k+1) -1 = 2M (n^2)^{2M}+1$. 
	In any sequence of $M$ "local steps" in a row in $\localrun$, 
	there is a $\quotemarks{\enregact}$ transition on every register in $I$. Suppose that $\length{\localrun} > \towerfun(n)(k+1)-1$. Additionally, suppose that  no local configuration appears twice in $\localrun$ (otherwise $\localrun$ may easily be shortened). 
	
	
	For every $i$, let $\atrans_i$ the $i$-th transition in $\localrun$, and let $\theta_i\in V_{in} \cup \set{\bot}$ be such that 
	\[
	\theta_i = 
	\begin{cases}
		& 	v \in V_{in} \text{ if the $i$th step of $u$ is a "reception step" } \step{\extlabel{\delta_i}{v}} \\ 
		& 	\bot \text{ otherwise}
	\end{cases}
	\]

	For every $i \in \nset{0}{n^{2M}}$, we write $s_i$ the sequence $\delta_{2  M \cdot i+1}, \delta_{2  M \cdot i+2}, \cdots, \delta_{2 M \cdot i+2M}$ and $\Theta_i$ the sequence $\theta_{2  M \cdot i}, \theta_{2 M \cdot i+1}, \cdots, \theta_{2 M \cdot i+2M}$.
	There are $|\transitions|^{2M}$ possible sequences for $s_i$ and $[\size{V_{in}} +1]^{2M}$ possible sequences for $\Theta_i$.
	By the pigeonhole principle there exist two indices $i_a, i_b$ such that the sequences $s_{i_a}$ and $s_{i_b}$ are equal (as $|\transitions| (\size{V_{in}}+1) \leq n^2$). 
	There exist two infix "local runs" $\localrun_a: (q_1, \localdata_1) \step{*} (q_2, \localdata_2)$, $\localrun_b: (q_3, \localdata_3) \step{*} (q_4, \localdata_4)$ in $\localrun$ such that $(q_2,\localdata_2)$ appears strictly before $(q_3,\localdata_3)$ in $\localrun$ and $\localrun_a$ and $\localrun_b$ both have the same sequences of transitions, which we call $s$, and of receptions of values from $V_{in}$, which we call $\Theta$.
	...
	We build a "trace" $\atrace$ such that $(q_1,\localdata_1) \step{\atrace} (q_4, \localdata_4)$ where the underlying sequence of transitions of $\atrace$ is $s$ and the receptions of values of $V_{in}$ matches $\Theta$.
	...
	Because $V_{in}$ and $V$ are finite, there exists an injective function $\phi: V \rightarrow \nats$ such that for all $v \in V_{in}$, $\phi(v) = v$ and for all $v \notin V_{in}$, $\phi(v) \notin V$.
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $\localdata_a(i) \in V_{in}$ hence $\localdata(i) = \phi(\localdata_a(i)) = \localdata_a(i) \neq \localdata_a(i')$
			
			\item $\localdata(i') \notin V_{in}$ therefore $\phi(\localdata_a(i)) \notin V$ and, as $\localdata(i') = \localdata_a(i') \notin V$,  $\localdata(i) \ne \localdata(i')$
		\end{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $\localdata_a(i) \in V_{in}$, hence the last reception step on register $i$ with $\enregact$ was with a value $v' \in V_{in}$, hence $\phi(\localdata_a(i)) = \phi(\localdata_b(i))=v'$, and $\localdata(i) = \phi(\localdata_a(i)) = \phi(\localdata_b(i)) = \localdata_b(i) \neq  \localdata_b(i') = \localdata(i')$, or
	...
	Let $\aval' \in \nats \setminus V_{in}$, if $\aval' \in V$ then by the previous argument $\vinput{\aval'}{\localrun'}$ is a subword of $\vinput{\aval'}{\localrun}$. If $\aval' \notin V$ then $\aval'$ does not appear in $\localrun$. Either $\aval'$ does not appear in $\localrun'$ in which case the desired property is true, or there exists $\aval \in V$ such that $\aval' = \phi(\aval)$. As $\phi$ is injective and $\phi(V_{in}) = V_{in}$ and $\aval'\notin V_{in}$, $\aval \notin V_{in}$. 
	...
	We build a "local run" of length strictly less that $\towerfun(\size{\prot})(\regnum)$ as follows. We start with $\localrun^{(0)} := \localrun$ and $V^{(0)}$ the set of values of messages appearing in $\localrun^{(0)}$. For every $k$ such that $\length{\localrun^{(0)}} > \towerfun(\size{\prot})(\regnum) -1$, we apply the "shortening property" on $\localrun^{(k)}$ and $V^{(k)}$ to obtain $\localrun^{(k+1)}$ and define $V^{(k+1)}$ by $V^{(k)} \cup W$ where $W$ is the set of values of messages in $\localrun^{(k+1)}$, which is finite.
	The construction stops when $\length{\localrun^{(k)}} \leq \towerfun(\size{\prot})(\regnum)-1$, which concludes the proof of Lemma~\ref{lem:short-local-runs}. 
\end{proof}
...
\begin{proof}
	We set $\psi$ to be the same function as in Lemma~\ref{lem:short-local-runs}. 
	Let $\prot$ be a protocol with $r$ registers, $V_{in}$ a finite set of values. 
	Let $n = \size{\prot} + \size{V_{in}} +1$.
	...
	\begin{enumerate}
		\item for all $\aval' \in \nats \setminus V_{in}$, there exists $\aval \in \nats \setminus V_{in}$ such that $\vinput{\aval'}{\localrun_0'}$ is a subword of $\vinput{\aval}{\localrun_0}$,
	...
	By Lemma~\ref{lem:short-local-runs}, there exists a "local run" $\localrun_1': (q_1, \localdata_1) \step{*} (q', \localdata')$ such that $\length{\localrun_1'} \leq \towerfun(\size{\prot})(r)-1$ and:
	\begin{enumerate}
		\item for all $\aval' \in \nats \setminus V_{in}$, there exists $\aval \in \nats \setminus V_{in}$ such that $\vinput{\aval'}{\localrun_1'}$ is a subword of $\vinput{\aval}{\localrun_1}$,
	...
	\begin{itemize}
		...
		\item if $v' \in V'' \setminus (V \cup V_{in})$ and then $v'$ does not appear in $\localrun$, and there exists $v \in \nats\setminus V_{in}$ such that $\vinput{v'}{\localrun_0'} \subword \vinput{v}{\localrun_0}$. Further, as $v' \in V'$ we have $\vinput{v'}{\localrun_1'} \subword \vinput{v'}{\localrun_1} = \epsilon$, hence $\vinput{v'}{\localrun'} \subword \vinput{v}{\localrun}$.
		
		\item if $v' \in \nats \setminus (V_{in} \cup V'')$, then by definition of $V''$ we have $\vinput{v'}{\localrun_0'} = \epsilon$, and there exists $v \in \nats \setminus V_{in}$ such that $\vinput{v'}{\localrun_1'} \subword \vinput{v}{\localrun_1}$, hence $\vinput{v'}{\localrun'} \subword \vinput{v}{\localrun}$.
	\end{itemize}

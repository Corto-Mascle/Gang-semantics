...
We replace the local run label of $\node$ with $u'$. 
	...
	The proof of the second point is analogous to the one for "boss nodes": we identify the necessary broadcasts in the local run and reduce the runs in-between them using Lemma~\ref{lem:short-local-runs}\corto{fix}. The only difference is that instead of broadcasting the letters of its boss label, $\node$ only has to broadcast the message $(\followlabelmessage{\node}, \valuelabel{\node})$ in addition to the messages required by its "follower" children, hence the $\size{\bosslabel{\node}}$ term is replaced by $1$ in the inequality.
\end{proof}

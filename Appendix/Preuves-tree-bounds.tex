...
\begin{proof}
	...
	\begin{align*}
	\size{\localrunlabel{\node}}& 
	\leq (\towerfun(\size{\prot},r)+1) f_0(\altmax-\altitude{\node})(\size{\messages} r +1) \\&  
...
\begin{proof}
	...
	We define $f_2'$ as $f_1 \circ f_0 \circ f_1$ and $f_2 : k \mapsto f_2'(k+2)$, so that $f_2(\size{\prot} + \size{w}) \geq f_1(f_0(f_1(N)+1))$. It is a function of $\Ffunction{\omega^{\size{\messages}+1}}$ as $f_0$ and $f_1$ are in $\Ffunction{\omega^{\size{\messages}}} \subseteq \Ffunction{\omega^{\size{\messages}+1}}$ and $\Ffunction{\omega^{\size{\messages}+1}}$ is closed by composition with functions of $\Ffunction{\omega^{\size{\messages}}}$.
\end{proof}

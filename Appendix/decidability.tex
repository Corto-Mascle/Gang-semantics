...
\begin{proof}
	...
	By Lemma~\ref{prop:bound-tree-size}, there is such a "unfolding tree" if and only if there is one of size at most $f(\size{\prot}+2)$, where $f$ is a function of the class $\Ffunction{\omega^{\size{\messages}}}$.
	...
	As a result, there exists a "unfolding tree" satisfying $m$ if and only if there is one of size at most $f(\size{\prot}+2)$ in which values do not exceed $f(\size{\prot}+2)$. A description of such a tree requires a space that is polynomial in its size.
	...
	An algorithm for \COVER thus consists in enumerating all trees labelled by local runs of $\prot$ of size at most $f(\size{\prot}+2)$ with values not exceeding $f(\size{\prot}+2)$, and accepting if and only if one of them satisfies the conditions to be a "unfolding tree" satisfying $m$.

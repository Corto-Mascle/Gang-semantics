...
\begin{proof}
	...
	An algorithm for \COVER thus consists in enumerating all trees labeled by local runs of $\prot$ of size at most $f(\size{\prot}+2)$ with values not exceeding $f(\size{\prot}+2)$, and accepting if and only if one of them satisfies the conditions to be a "unfolding tree" satisfying $m$.

...
\begin{definition}
	Given a set of states $K \subseteq Q$, we say that a pair $\config, v$ with  $\config$ a "configuration" over a set of agents $\agents$ and $v \in \nats$ satisfies $K$ if for all $q \in K$, there exists $a \in \agents$ such that $\config(a) = (q,v)$.
	A configuration $\config$ satisfies $K$ if there exists $v\in \nats$ such that $\config, v$ satisfies $K$. 

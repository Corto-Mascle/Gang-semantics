%\begin{definition}
%	Given two runs $\r, \r'$ over sets of agents $A, A'$ respectively, we define their ""parallel composition"" as the run over $A\times \set{0} \cup A'\times \set{1}$ defined as follows: 
%\end{definition}


\begin{lemma}[Copycat]\label{lem:copycat}
%	Let $\r$ be an initial run over a set of agents $A$, for all $n$ we can define an initial run $\r'$ over $A \times \set{1,\ldots, n}$ such that:
%	\begin{itemize}
%		\item   
%		
%		\item

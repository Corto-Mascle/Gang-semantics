...
\begin{restatable}{lemma}{lemShortRunOutput}
	...
	for every "local run" $\localrun: (q, \localdata) \step{*} (q', \localdata')$ such that $w_{out} \subword \Output{\localrun}$ and for every $V \subseteq \nats$ finite such that $V$ contains all message values appearing in $\localrun$, there exists a "local run" $\localrun': (q, \localdata) \step{*} (q', \localdata')$ such that, denoting by $\Vinit$ the set of initial values in $\localrun$, we have $\length{\localrun'} \leq \towerfun(\size{\prot}+\regnum, \regnum) (\size{w_{out}}+1)$, $w_{out} \subword \Output{\localrun'}$ and:
	\begin{enumerate}
...
\begin{restatable}{lemma}{lemBoundSuccessorHeight}
	...
	Let $\prot$ be a "protocol" over $\regnum$ registers, let $\node$ be a node of an "unfolding tree" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$.
	...
	\end{enumerate}
\end{restatable}
\luin{sans doute qu'il faut changer l'énoncé de ce lemme pour la partie corps du papier}

\subsection{Tree bounds}
\label{sec:tree-bounds}

\luin{ici il faut de l'intuition + peut etre une figure qui illustre comment on range notre arbre}
\begin{figure}
...
\luin{mentionner que c'est dans le lemme suivant que l'on utilise le length theorem}

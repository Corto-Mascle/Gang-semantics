...
\begin{lemma}
	...
	There exists a function $\towerfun(n, e)$ in $O(\repexp{n}{e})$ such that, for every protocol $\prot$ with $r$ registers per process, for every "local run" $\localrun: (q, \localdata) \step{*} (q', \localdata')$ in $\prot$, there exists $\localrun': (q, \localdata) \step{*} (q', \localdata')$ such that $\length{\localrun'} \leq \towerfun(\size{\prot})(r)$ and:
	\begin{enumerate}
...
Let a "local run" $\localrun: (q_i, \localdata_i) \step{*} (q_f, \localdata_f)$ with $k$ "active registers" such that $\length{\localrun} > \towerfun(n,k)$ and let $V \subseteq \nats$ finite that contains every message value appearing in $\localrun$. We claim that $\localrun$ can be shortened into a local run $\localrun': (q_i, \localdata_i) \step{*} (q_f, \localdata_f)$ with $k$ active registees such that $\length{\localrun'} < \length{\localrun}$ and:
 \begin{itemize}
\item for all $\aval' \in \nats$, there exists $\aval \in \nats$ such that $\vinput{\aval'}{\localrun'}$ is a subword of $\vinput{\aval}{\localrun}$,
		\item for all $\aval \in V$, $\vinput{\aval}{\localrun'}$ is a subword of $\vinput{\aval}{\localrun}$. 
 \end{itemize}

We proceed by induction on the number $k$ of "active registers" in the "local run". If $k=0$, registers keep their values throughout $\localrun$. Therefore, if $\localrun$ goes through the same state twice, all steps in between may be removed, and we take $\towerfun(\size{\prot}, 0) := |\prot| + 1 \geq |Q| +1$.  
...
there is a $\quotemarks{\enregact}$ transition on every register in $I$. Let $\towerfun(n)(k+1) := M (n^{2M}+1)$ and suppose that $\length{\localrun} > \towerfun(n)(k+1)$. Additionally, suppose that $\localrun$ does not go twice through the same local configuration (otherwise it may be easily shortened). For every $i$, let $\atrans_i$ the $i$-th transition in $\localrun$.
...
For every $j \in \nset{1}{2M}$, let $\delta_j$ denote the $j$-th transition of $s$. First, if  $\delta_j$ is a "broadcast" or a "local test", we define "internal step" $j$ of $\sigma$ to be $\intlabel{\delta_j}$. 
...
For every $i \in I$, let $\localdata_a, \localdata_b$ the content of the register after step $l$ in $\localrun_a$ and $\localrun_b$ respectively.
...
\begin{itemize}
\item $l+1<e_i$: $\localdata(i)= \localdata_a(i)$ and $\aval = \aval_a$, and because the "internal step" is valid in $\localrun_a$ we have $\binrel{\localdata_a(i)}{\anact}{\aval_a}$. 
\item $e_i \leq l+1 < f_i$: We have $\aval = \phi(\aval_a)$. 
Moreover, because $e_i < l+1$, there is at least one $\quotemarks{\enregact}$ on register $i$ in $\sigma_l$. Consider the last such "transition" in $\sigma_l$; its index $j$ satisfies $e_i \leq j < f_i$ by definition of $e_i$, hence the value of the corresponding "external message" in $\localrun_a$ is $\localdata_a(i)$ and its value in $\sigma_{l+1}$ is $\phi(\localdata_a(i))$. One has $\binrel{\localdata_a(i)}{\anact}{\aval_a}$ therefore (by injectivity of $\phi$ for $\anact = \quotemarks{\diseqtestact}$)
...
If $i \notin I$ and $i' \notin I$ (none are "active registers") then there values are the same as in $\localrun$ and, because the "local test" was satisfied in $\localrun$ we have $\localdata(i) \ne \localdata(i')$.

If $i \in I$ and $i' \notin I$, then:
\begin{itemize}
\item if $l+1<e_i$ then $\localdata(i) = \localdata_a(i) \ne \localdata(i')$,
\item if $e_i \leq l+1 < f_i$ then $\localdata(i) = \phi(\localdata_a(i)) \in V$ and  $\localdata(i') \notin V$ therefore $\localdata(i) \ne \localdata(i')$,
\item if $f_i \leq l+1$ then $\localdata(i) = \localdata_b(i) \ne \localdata_b(i')$.
\end{itemize}

If $i \in I$ and $i' \in I$, recall that $e_i \leq M < f_{i'}$ and $e_{i'} \leq M < f_i$. Therefore, we again make a case disjunction on the value of $l+1$:
\begin{itemize}
\item $l+1<e_i$ and $l+1 <e_{i'}$: $\localdata(i)= \localdata_a(i)$ and $\localdata(i)= \localdata_a(i)$, and because the "internal step" is valid in $\localrun_a$ we have $\localdata_a(i) \ne \localdata_a(i')$. 
\item $e_i \leq l+1 < f_i$, $l+1<e_{i'}$: $\localdata(i) = \phi(\localdata_a(i)) \in \nats  \setminus V$, $\localdata(i') = \localdata_a(i') \in V$ therefore $\localdata(i') \ne \localdata(i)$.
...
\item $e_i \leq l+1 < f_i$, $f_{i'} \leq l+1$: $\localdata(i') = \localdata_b(i') \in V$, $\localdata(i) = \phi(\localdata_a(i) \in V$ hence $\localdata(i) \ne \localdata(i')$.
\item $f_i \leq l+1$, $e_{i'} \leq l+1 < f_{i'}$: symmetric to the previous case.
\item $f_i \leq l+1$, $f_{i'} \leq l+1$: $\localdata(i)= \localdata_b(i)$ and $\localdata(i)= \localdata_b(i)$, and because the "internal step" is valid in $\localrun_b$ we have $\localdata_b(i) \ne \localdata_b(i')$. 
\end{itemize}
...
It remains to prove the conditions on the "$v$-inputs" of $\localrun'$. If suffices to prove the condition for the part between $(q_1, \localdata_1)$ to $(q_4, \localdata_4)$,
 because the rest of $\localrun$ is left untouched. Let $\tau$ the "schedule" of the suffix run of $\localrun$ from $(q_1, \localdata_1)$ to $(q_4, \localdata_4)$, so $(q_1,\localdata_1) \step{\tau} (q_4, \localdata_4)$. 

Let $(q_m, \localdata_m)$ the "local configuration" after $M$ steps of $\sigma$ from $(q_1, \localdata_1)$; write $\localrun_1$ the local run from $(q_1,\localdata_1)$ to $(q_m, \localdata_m)$ corresponding to the first $M$ steps of $\sigma$, and $\localrun_2$ the "local run" from $(q_m, \localdata_m)$ to $(q_4, \localdata_4)$ corresponding to the last $M$ steps of $\sigma$.  

Let $\aval \in V$. We claim that $\vinput{\aval}{\localrun_{1}}$ is a subword of $\vinput{\aval}{\localrun_a}$ and $\vinput{\aval}{\localrun_{2}}$ is a subword of $\vinput{\aval}{\localrun_b}$. Indeed, in the construction of $\sigma$, the "external message" steps were those of $\localrun_a$ except that some values were replaced with fresh values in $\nats \setminus V$, and similarly with $\localrun_b$ and the second half. Overall, this proves that $\vinput{\aval}{\localrun'}$ is a subword of $\vinput{\aval}{\localrun}$ for every $\aval in V$. Note that this proves both required conditions for values in $V$, \emph{i.e.}, values appearing in $\localrun$.

Let $\aval' \in \nats \setminus V$. Either $\aval'$ does not appear in $\sigma$ in which case the desired property is true, or there exists $\aval \in V$ such that $\aval' = \phi(\aval)$. But then $\vinput{\aval'}{\localrun_{1}}$ is a subword of $\vinput{\aval}{\localrun_a}$ and $\vinput{\aval'}{\localrun_{2}}$ is a subword of $\vinput{\aval}{\localrun_b}$. Indeed, in $\localrun_1$, the "external message" steps with value $\phi(\aval)$ correspond to "external message" steps in $\localrun_a$ with value $\aval$, and similarly for the second half with $\localrun_b$. This proves the condition for every $\aval'$ that does not appear in $\localrun$.
...
\begin{lemma}
	\label{lem:short-dec}
	Let $\decsymb = (w_0, m_1, \ldots, w_\ell)$ be a decomposition, and for all $i\in [0,\ell]$ let $\decsymb_i = (w_0, \ldots, w_i)$.
	Let $w'_0, \ldots, w'_\ell$ be such that $w'_i \in \langdecdown{\decsymb_i}$ for all $i$.
	
	Then there exists $\widehat{\decsymb} = (\widehat{w}_0, m_1, \ldots, \widehat{w}_\ell)$ such that $w'_i \in \langdecdown{\widehat{\decsymb}_i}$ (where $\widehat{\decsymb}_i = (\widehat{w}_0, m_1, \ldots, \widehat{w}_\ell)$) for all $i$ and $\sum_{i=1}^{\ell} \size{\widehat{w}_i} \leq \sum_{i=1}^{\ell} \size{w'_i}$. 
\end{lemma}
...
\begin{proof}
	...
	As a result, as $\tree$ is of minimal size \corto{define size of a tree and of a protocol}, we have $k \leq \size{\messages}(r+1)$.
...
\begin{lemma}
	...
	There exists a function $f_1$ of the class $F_{\omega^{\size{\messages} +1}}$ such that for all "protocol" $\prot$, for all "tree unfolding" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$, the maximal altitude of a node of $\tree$ is bounded by $f_1(\size{\prot} + \size{w})$.
\end{lemma}
...
\begin{corollary}
	There exists a function $f_2$ of the class $F_{\omega^{\size{\messages} +1}}$ such that for all "protocol" $\prot$, for all "tree unfolding" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$, if we call $\node$ its root then $\size{\bosslabel{\node}}$ and $\size{\localrunlabel{\node}}$ are both bounded by $f_2(\size{\prot} + \size{w})$.
\end{corollary}

\ifproofs
\begin{proof}
	Consequence of Lemmas~\ref{lem:bound-length-at-height-h} and~\ref{lem:bound-max-height} as $F_{\omega^{\size{\messages}+1}}$ is closed by composition with primitive recursive functions.
\end{proof}
\fi

\begin{lemma}
	\label{lem:bound-min-height}
	There exists a function $f_3$ of the class $F_{\omega^{\size{\messages} +1}}$ such that for all "protocol" $\prot$, for all "tree unfolding" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$, the absolute value of the minimal "altitude" of a node of $\tree$ is bounded by $f_3(\size{\prot} + \size{w})$.
\end{lemma}
...
\begin{corollary}
	...
	There exists a function $f_4$ of the class $F_{\omega^{\size{\messages} +1}}$ such that for all "protocol" $\prot$, for all "tree unfolding" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$, for all node $\node$ of $\tree$,
	
		\begin{itemize}
		\item If $\node$ is a "boss node" then $\size{\bosslabel{\node}} \leq \size{\localrunlabel{\node}} \leq f_4(\size{\prot}+ \size{w})$.
...
\ifproofs
\begin{proof}
	Consequence of Lemmas~\ref{lem:bound-length-at-height-h}, \ref{lem:bound-max-height} and \ref{lem:bound-min-height} as $F_{\omega^{\size{\messages}+1}}$ is closed by composition with primitive recursive functions.
\end{proof}
\fi


\begin{proposition}
	There exists a function $f_5$ of the class $F_{\omega^{\size{\messages} +1}}$ such that for all "protocol" $\prot$, for all "tree unfolding" $\tree$ of minimal size labelled by $\prot$ satisfying a "boss specification" $w$, the size of $\tree$ is bounded by $f_5(\size{\prot} + \size{w})$.
\end{proposition}
...
\begin{theorem}
	The BNRA coverability problem is decidable in time $F_{\omega^\omega}$\corto{proper notation} and there is no $\alpha < \omega^\omega$ such that this problem can be solved in time $F_{\alpha}$.

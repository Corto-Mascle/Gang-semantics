...
\begin{example}
	In \cref{fig-ex-unfolding-tree}~we display an "unfolding tree" obtained from the run of \cref{ex:example-1}. Each node is labeled with a local run, a specification, and a value (we omit the labels of the root). Local configurations are displayed as a state among with its two registers' values. For instance, the local run of the node at the root of the tree is \begin{multline*}
...
%	\luin{ici explications sur chacune des conditions}
	...
	The right node received $m_2$ among with $x_1$, it can then broadcast $m_2$ (which shall not be received) and reach $q_4$. From $q_4$ it can send message $m_6$ among with $x_1$. The message is received by the agent playing the root node role, allowing him to reach $q_7$.

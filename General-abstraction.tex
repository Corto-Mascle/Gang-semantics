...
The ""size"" of an "unfolding tree" is defined as the sum of the lengths of the local run labels and specifications of all its nodes.
...
\label{lem:shortening-branches}
	...
	then $\tree$ can be shortened into an "unfolding tree" $\tree'$ of smaller size whose root's local run also covers $q_f$. 
\end{restatable}
...
In this section we show that there is a computable bound on the size of the "unfolding tree" achieving a given specification and labelled with a "protocol" $\prot$. Lemma~\ref{lem:shortening-branches} leads us towards an application of the "Length function theorem". 

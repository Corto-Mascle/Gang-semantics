...
\begin{definition}
	...
	\begin{itemize}
		...
		\item The third one is a ""specification"", which is either a word $\bosslabel{\node} \in \messages^*$ (boss specification) or a pair $(\followlabelword{\node}, \followlabelmessage{\node}) \in \messages^* \times \messages$ (follower specification). In the first case we say that the node is a ""boss node"", otherwise it is a ""follower node"".
		When we do not want to specify the type of specification, we write $\speclabel{\node}$ for the specification label.
	\end{itemize} 
	
	It must satisfy the following conditions:
	Let $\node$ be a node of that tree. The following conditions must hold:
	
	\begin{itemize}
		\item[C1\namedlabel{unfoldingC1}{C1}] If it is a "boss node" then $\valuelabel{\node}$ is an initial value of $\localrunlabel{\node}$ and there exists a decomposition $\decsymb = (w_0, m_1, w_1, \ldots, m_{\ell}, w_{\ell})$ such that
	 $\voutput{\valuelabel{\node}}{\localrunlabel{\node}} = w_0 \cdots w_\ell$, $\bosslabel{\node} \in \langdec{\decsymb}$, and for all $j \in [1,\ell]$, $\node$ has a child $\node'$ which is a "follower node" such that $\followlabelmessage{\node'} = m_j$ and $\followlabelword{\node'} \in\langdecdown{\decsymb_j}$ where $\decsymb_j = (w_0, m_1, w_1, \ldots, m_{j-1}, w_{j-1})$.
		
		\item[C2\namedlabel{unfoldingC2}{C2}] If $\node$ is a "follower node" then $\valuelabel{\node}$ is not an initial value of $\localrunlabel{\node}$, $\vinput{\valuelabel{\node}}{\localrunlabel{\node}}$, and $\followlabelmessage{\node}$ is broadcast with value $\valuelabel{\node}$ at some point in $\localrunlabel{\node}$.
		
		\item[C3\namedlabel{unfoldingC3}{C3}] In both cases, for all value $\val' \neq \valuelabel{\node}$ broadcast or received at some point in $\localrunlabel{\node}$,
		\begin{itemize}
			\item[C3.1\namedlabel{unfoldingC3.1}{C3.1}] If $v'$ is an initial value of $\localrunlabel{\node}$ then there exists a "decomposition" $\decsymb' = (w'_0, m'_1, w'_1, \ldots, m'_k, w'_k)$ where $w'_0 w'_1 \ldots w'_k = \voutput{\val'}{\localrunlabel{\node}}$, for all $j \in [1,k]$ all receptions of $m'_j$ with value $\val'$ in $u$ happen after the broadcasts of $w'_0\cdots w'_{j-1}$ with value $\val'$, and for all $j \in [1,k]$ $\node$ has a child $\mu'$ which is a "follower node" such that $\followlabelmessage{\node'} = m'_j$ and $\followlabelword{\node'} \in\langdecdown{\decsymb'_j}$ where $\decsymb'_j = (w_0, m_1, w_1, \ldots, m_{j-1}, w_{j-1})$.  
			
			\item[C3.2\namedlabel{unfoldingC3.2}{C3.2}] If $\val'$ is not an initial value of $\localrunlabel{\node}$ then $\node$ has a child $\node'$ which is a "boss node" such that $\vinput{\val'}{\localrunlabel{\node}}$ is a subword of $\bosslabel{\node'}$.
		\end{itemize}
	\end{itemize}
...
%		\begin{itemize}
%			\item[C3.1\namedlabel{unfoldingC3.1}{C3.1}] If $v'$ is an initial value of $u$ then there exists a "decomposition" $\decsymb' = (w'_0, m'_1, w'_1, \ldots, m'_k, w'_k)$ where $w'_0 w'_1 \ldots w'_k$ is the $v'$-output of $u$, for all $j \in [1,k]$ all receptions of $m_j$ with value $v'$ in $u$ happen after the broadcasts of $w'_0\cdots w'_{j-1}$ with value $v'$, and for all $j \in [1,k]$ $\mu$ has a child which is a "follower node" with a label $\decsymb \subword ((w_0, m_1, w_1, \ldots, m_{j-1}, w_{j-1}), m_j)$.  
%			
%			\item[C3.2\namedlabel{unfoldingC3.2}{C3.2}] If $v'$ is not an initial value of $u$ then $\mu$ has a child which is a "boss node" labelled by $w'$ such that $\vinput{v}{u}$ is a subword of $w'$.
%		\end{itemize}
%	\end{itemize}
%	
%	

...
Thanks to Proposition~\ref{prop:loc-eq-test-elimination}, we may assume that our protocols have no "local equality tests" (the complexity class $\Fomegaomega$ is stable by exponential reduction). \corto{again, only useful for Tower}

The intuition of our decidability procedure is as follows. 
Consider a run where an agent $a$ covers some state $q$; we want to understand what $a$ needs from other agents in order to cover $q$. For instance, it might need to receive a sequence $w \in \messages^*$ of messages all with the same value, so that it stores the value of the first such message and tests further messages for equality.
...
We will proceed as follows. In Section~\ref{sec:decidability-defs}, we introduce several useful notions. In Section~\ref{sec:decidability-tree-unfoldings}, we define the notion of "unfolding tree". In Section~\ref{sec:tree-bounds}, we bound the size of the "unfolding trees" that we have to consider; this is in fact the most technical part of this article. Finally, in Section~\ref{sec:decidability-end}, we conclude by exposing our decidability procedure. 


\subsection{Useful definitions}
\label{sec:decidability-defs}

We define a notion of "local run", which may be seen as the projection of a "run" onto a given agent. In this local vision, we do not consider the origin of the messages received.
...
\AP A ""reception step"" from $(q,\localdata)$ to $(q',\localdata')$ with transition $\atrans \in \transitions$ and value $\aval \in \nats$, denoted $(q,\localdata) \extbr{\atrans}{\aval} (q',\localdata')$, is defined when $\atrans$ is of the form $(q,\rec{m}{j}{\anact},q')$ with $\localdata(j') = \localdata'(j')$ for all $j' \neq j$ and one of the following cases holds:
	...
	Intuitively this describes the sequence of messages sent with some value $v$ in a run: The $w_i$ are messages sent by the agent with that value initially, and the $m_i$ mark the times at which each message type is broadcast for the first time by another agent. This definition will be crucial to derive from a "local run" a set of "specifications" that make it feasible.
...
\begin{example}
	In \cref{fig-ex-unfolding-tree}~we display an "unfolding tree" obtained from the run of \cref{ex:example-1}. Each node is labeled with a local run, a specification, and a value (we omit the labels of the root). Local configurations are displayed as a state along with its two registers' values. For instance, the local run of the node at the root of the tree is \begin{multline*}
...
%	\luin{ici explications sur chacune des conditions}
	...
	Note that if one wants to build a concrete execution of the protocol of Figure \ref{fig:ex1}~from the tree of Figure \ref{fig-ex-unfolding-tree}, the build execution should be over 4 agents (one agent per node). Each agent will play the role of a node of the tree. The agent playing the role of the bottom left node should broadcast its value along with message $m_2$ and stop. This message should be received by the agent playing the role of the boss node with value $y_2$, it will reach $q_5$ and store the value $x_3$ in its first register. It will then broadcast message $m_4$ along with value $y_2$ and stop. The latter broadcast should be received by the agent playing the role of the root node. This agent has broadcast messages $m_2$ before and reached $q_1$. When he broadcast message $m_2$ along with its value $x_1$, the message has been received by the agent playing role of the right node.
	The right node received $m_2$ along with $x_1$, it can then broadcast $m_2$ (which shall not be received) and reach $q_4$. From $q_4$ it can send message $m_6$ along with $x_1$. The message is received by the agent playing the root node role, allowing him to reach $q_7$.
...
The translation from tree to run consists in an induction on the tree. A key concept is the one of "partial run", which is a run in which some receptions are not matched by a broadcast, and thus the run needs to be completed with some messages from another set of agents.
 Let $\tau$ be an unfolding tree. We show that if its root is labeled by a boss specification $\bossspec$ then there is a run in which $\bossspec$ is broadcast with a constant value. If it is labeled by a follower specification $(\followwordspec, \followmessagespec)$ then there is a "partial run" which requires a sequence of broadcasts of $\followwordspec$ over a constant value to broadcast $\followmessagespec$ itself with that same value.
 The induction then works by considering the runs obtained from the children of the root by induction hypothesis and then constructing a run using a parallel composition of those runs with the local run labeling the root to eliminate the receptions without broadcasts and output the required sequence of messages. 
\end{proof}

...
\begin{definition}[Protocols]
	A ""protocol"" with $\regnum$ registers is a tuple $\prot = (Q, \messages, \transitions, q_0)$  with $Q$ a finite set of states, $q_0 \in Q$ an initial state, $\messages$ a finite set of ""messages""  and $\transitions \subseteq Q \times \operations \times Q$ a finite set of transitions, with a set of operations 
	\begin{multline*}
...
\begin{example}\label{example-2}
	Let $\prot$ the protocol of \ref{fig:ex1}. As proven in Example~\ref{ex:example1}, $(\prot,q_7)$ is a positive instance of \COVER. However, $(\prot,q_7)$ is a negative instance of \TARGET: for an agent to reach $q_7$, there must be an agent staying on $q_6$ to broadcast $m_6$. However $(\prot,q_1)$ is a positive instance of \TARGET: from an initial configuration all agents can broadcast $m_1$ one at a time. Also, $(\prot, q_3)$ is a negative instance of \COVER: we would need an agent $a$ on $q_2$ and an agent $a'$ on $q_5$ with the same value in their first registers. The value of the first register of $a$ comes from a broadcast of message type $m_1$ where it comes from a broadcast of message type $m_2$ for $a'$. An agent broadcasting message type $m_1$ or $m_2$ must broadcast it with one of its initial value and may not broadcast both $m_1$ and $m_2$ in the same run; therefore $a$ and $a'$ cannot have the same value in their first register and $q_3$ cannot be covered. 
\end{example}

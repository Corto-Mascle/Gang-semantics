...
\begin{definition}[Syntax]
	A ""protocol"" with $r$ registers is a tuple $\prot = (Q, \messages, \transitions, q_0)$  with $Q$ a finite set of states, $q_0 \in Q$ an initial state, $\messages$ a finite set of ""messages""  and $\transitions \subseteq Q \times Op^\messages_r \times Q$ a finite set of transitions, with $Op^{\messages}_r = \set{\br{\amessage}{i}, \rec{\amessage}{i}{\dummyact}, \rec{\amessage}{i}{\enregact}, \rec{\amessage}{i}{\eqtestact}, \rec{\amessage}{i}{\diseqtestact}, \loc{i}{j}{\eqtestact}, \loc{i}{j}{\diseqtestact} \mid \amessage \in \messages, 1 \leq i \leq r}$ the set of operations.
...
\begin{lemma}
	...
	Furthermore if $r = 1$ then $\prot'$ only needs one register.
\end{lemma}

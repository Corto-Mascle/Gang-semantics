...
\begin{example}\label{example-1}
	We give an example of a "protocol" with 2 registers in Figure \ref{fig:ex1}. Its set of states is $\{q_i\}_{0 \leq i \leq 6}$, and the initial state is $q_0$. Let $\agents = \{a_1, a_2\}$ and let $(x_i, y_i)_{1 \leq i \leq 2} \in \mathbb{N}$ be 4 different integers. The function $\config_0$ associating to each agent $a_i$ the element $(q_0, (x_i, y_i))$ is an initial configuration over $\agents$. There exists a run covering $q_7$: from $\config_0$, let agent $a_1$ broadcast message $m_2$ among with its value $x_1$, and let agent $a_2$ receives it with the approriate transition. The reached configuration $\config_1$ should be such that: $\st{\config_1}(a_1)= q_1$, $\st{\config_1}(a_2)= q_5$, $\data{\config_1}(a_1)(1) = \data{\config_1}(a_2)(1) = x_1$. Let now agent $a_2$ broadcast message $m_4$ among with its value $y_2$ and let agent $a_1$ receives the message. The resulting configuration $\config_2$ is such that $\st{\config_2}(a_1)= q_4 = \st{\config_2}(a_2)$, $\data{\config_2}(a_2)(2) = \data{\config_2}(a_4)(2) = y_2$. From $\config_2$, agent $a_2$ can broadcast message $m_6$ among with its value $x_1$, let agent $a_1$ receives its value, as $\data{\config_2}(a_1)(1) = x_1 =\data{\config_2}(a_2)(1)$, agent $a_1$ can take the reception transition leading to $q_7$. Hence, the resulting configuration $\config_3$ is such that $\st{\config_3}(a_1)= q_7$. 

...
\begin{example}\label{ex:example-1}
	...
	Let $\agents = \{a_1, a_2\}$. We denote a configuration $\config$ over $\agents$ by $\tuple{(\st{\config}(a_1), \data{\config}(a_1)), (\st{\config}(a_2),\data{\config}(a_2))}$. The following is a "run", where $x_1,y_1,x_2,y_2$ are distinct natural integers:
	\begin{multline*}

...
\begin{example}\label{ex:example-1}
	...
	Let $\agents = \{a_1, a_2\}$. We denote a "configuration" $\config$ over $\agents$ by $\tuple{(\st{\config}(a_1), \data{\config}(a_1)), (\st{\config}(a_2),\data{\config}(a_2))}$. The following sequence is a "run", where $x_1,y_1,x_2,y_2$ are distinct natural integers: \vspace{-0,3cm}
	\begin{multline*}
...
\begin{definition}
	...
	\AP The ""target reachability problem"" \TARGET asks, given a "protocol" $\prot$ and a state $q_f$, whether there is a "run" $\config_0 \step{*} \config_f$ of $\prot$ such that $\st{\config_f}(\agents) = \{q_f\}$, i.e all agents end on $q_f$.
\end{definition}

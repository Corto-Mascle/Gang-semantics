...
\begin{definition}[Semantics]
	...
	Given a "run" $\run$, we write $\agentsof{\run}$ its set of agents, $\statesin{\run}:= \set{q \in Q \mid \exists i, \exists a, \st{\config_i}(a) = q}$ the set of states appearing in $\run$ and $\valsof{\run} := \set{\aval \in \nats \mid \exists i,j, a, \data{\config_i}(a,j) = \aval}$.  
...
\begin{example}\label{ex:example-1}
	...
	Let $\agents = \{a_1, a_2\}$. We denote a configuration $\config$ over $\agents$ by $\tuple{(\st{\config}(a_1), \data{\config}(a_1)), (\st{\config}(q_2),\data{\config}(a_2))}$. The following "run" covers $q_7$, where $x_1,y_1,x_2,y_2$ are distinct natural integers:
	\begin{multline*}
	\tuple{(q_0, (x_1,y_1)), (q_0,(x_2,y_2))} \step{} \tuple{(q_1, (x_1,y_1)), (q_5,(x_1,y_2))} \step{} \\ \tuple{(q_4, (x_1,y_2)), (q_4,(x_1,y_2))} \step{} \tuple{(q_6, (x_1,y_2)), (q_4,(x_1,y_2))}
	\end{multline*}
	The broadcast messages are: $(m_2,x_1)$ by $a_1$, $(m_4,y_2)$ by $a_2$ and $(m_6,x_1)$ by $a_1$. Each broadcast is received by the other agent. 
\end{example}
...
\begin{example}\label{example-2}
	Let $\prot$ the "protocol" of Example~\ref{fig:ex1}. As proven in Example~\ref{ex:example-1}, $(\prot,q_7)$ is a positive instance of \COVER. However, $(\prot,q_7)$ is a negative instance of \TARGET: for an agent to reach $q_7$, there must be an agent staying on $q_6$ to broadcast $m_6$. Meanwhile, $(\prot,q_1)$ is a positive instance of \TARGET: all agents can broadcast $m_1$ to get to $q_1$. Also, $(\prot, q_3)$ is a negative instance of \COVER: we would need one agent on $q_1$ and one on $q_5$ with the same value in their first registers, hence we need a broadcast $(m_1,v)$ and a broadcast $(m_2,v)$ for some $v$. However, an agent broadcasting message $(m_1,v)$ or $(m_2,v)$ has $v$ as initial value; it cannot come back to $q_0$ thus cannot broadcast both $(m_1,v)$ and $(m_2,v)$ in the same "run". 
\end{example}

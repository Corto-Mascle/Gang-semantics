...
A ""Broadcast Network of Register Automata"" (BNRA) is defined by a finite transition system describing the behaviour of an agent, which can broadcast and receive messages with integer values, store them in local registers, or make equality tests.
...
We use $r \geq 0$ to denote the number of registers in each node. We consider a finite alphabet $\messages$ of messages.

\begin{definition}[Syntax]
	A ""protocol"" with $r$ registers is a tuple $\prot = (Q, \messages, \transitions, q_0)$  with $Q$ a finite set of states, $q_0 \in Q$ an initial state, $\messages$ a finite set of ""messages""  and $\transitions \subseteq Q \times Op^\messages_r \times Q$ a finite set of transitions, with 
	\[
	Op^{\messages}_r = \set{\br{\amessage}{i}, \rec{\amessage}{i}{\dummyact}, \rec{\amessage}{i}{\enregact}, \rec{\amessage}{i}{\eqtestact}, \rec{\amessage}{i}{\diseqtestact}, \loc{i}{j}{\eqtestact}, \loc{i}{j}{\diseqtestact} \mid \amessage \in \messages, 1 \leq i,j \leq r}
	\] 
	...
	A ""simple protocol"" is one that does not have any local tests or receptions with $\dummyact$.
...
\begin{definition}[Semantics]
	...
	An \emph{initial configuration} $\config$ is one where for all $a \in \agents$, $\st{\config}(a) = q_0$ and for all $a, a' \in \agents$, $i, i' \in [1,r]$, if $\data{\config}(a, i) = \data{\config}(a', i')$ then $a=a'$ and $i=i'$.
	...
	\AP Given two configurations $\config, \config' \in \configs{\agents}$, a ""step"" $\config \step{} \config'$ is defined when one of the two following cases happens:
	\begin{itemize}
		\item there exists $a \in \agents$ and a "local test" $(\st{\config}(a), \loc{j_1}{j_2}{\anact}, \st{\config'}(a))$ such that for all $a'\neq a$, $\config(a') = \config'(a')$ and
		\begin{itemize}
			\item either $\anact = \quotemarks{\eqtestact}$ $\data{\config}(a,j_1) = \data{\config}(a,j_2)$
			
			\item either $\anact = \quotemarks{\diseqtestact}$ $\data{\config}(a,j_1) \neq \data{\config}(a,j_2)$
		\end{itemize}
		
		\item there exist $\amessage \in \messages$, $a_0 \in \agents$ and $j_0 \in [1,r]$ such that $(\st{\config}(a_0),\br{\amessage}{j_0}, \st{\config'}(a_0)) \in \transitions$, $\data{\config}(a_0) = \data{\config'}(a_0)$ and for all $a \in \agents\setminus \set{a_0}$, either $\config'(a) = \config(a)$ or there exists a transition $(\st{\config}(a),\rec{m}{j}{\anact},\st{\config'}(a))$

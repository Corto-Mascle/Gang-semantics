...
\begin{example}\label{ex:example-1}
	...
	% Its set of states is $\{q_i\}_{0 \leq i \leq 6}$, and the initial state is $q_0$. 
	Let $\agents = \{a_1, a_2\}$. Initial configurations $\config_0$ over $\agents$ are such that $\config_0(a_i) = (q_0, (x_i, y_i))$ with $x_1,x_2,y_1,y_2 \in \nats$ all distinct. There exists a run covering $q_7$: from $\config_0$, agent $a_1$ broadcasts message $(m_2,x_1)$, which agent $a_2$ receives. This brings us to $\config_1$ such that $\st{\config_1}(a_1)= q_1$, $\st{\config_1}(a_2)= q_5$, $\data{\config_1}(a_1)(1) = \data{\config_1}(a_2)(1) = x_1$. Then, agent $a_2$ broadcasts message $(m_4,y_2)$ and agent $a_1$ receives it; we get to $\config_2$ where both agents are on $q_4$ and $\data{\config_2}(a_2)(2) = \data{\config_2}(a_4)(2) = y_2$. Finally, agent $a_2$ broadcasts message $(m_6,x_1)$, and agent $a_1$ receives it to go to $q_7$, which is possible as $\data{\config_2}(a_1)(1) = x_1 =\data{\config_2}(a_2)(1)$. 
...
\begin{definition}
	...
	\AP The ""cover problem"" is to determine, given a "protocol" and a state $q_f$, whether there is a "run" of this "protocol" whose last "configuration" has an agent in state $q_f$.
	
	\AP The ""target reachability problem"" is to determine, given a "protocol" and a state $q_f$, whether there is a "run" of this "protocol" with a non-empty set of agents whose last "configuration" $\config$ is such that $\st{\config}(\agentsof{\gamma}) = \{q_f\}$, i.e all agents reached $q_f$.
\end{definition}

\begin{example}\label{example-2}
	Let $\prot$ the "protocol" of \ref{fig:ex1}. As proven in Example~\ref{ex:example-1}, $(\prot,q_7)$ is a positive instance of \COVER. However, $(\prot,q_7)$ is a negative instance of \TARGET: for an agent to reach $q_7$, there must be an agent staying on $q_6$ to broadcast $m_6$. Meanwhile, $(\prot,q_1)$ is a positive instance of \TARGET: from an initial configuration all agents can broadcast $m_1$ one at a time. Also, $(\prot, q_3)$ is a negative instance of \COVER: we would need an agent $a$ on $q_2$ and an agent $a'$ on $q_5$ with the same value in their first registers. The value of the first register of $a$ comes from a broadcast of message type $m_1$ where it comes from a broadcast of message type $m_2$ for $a'$. An agent broadcasting message type $m_1$ or $m_2$ must broadcast it with one of its initial value and may not broadcast both $m_1$ and $m_2$ in the same run; therefore $a$ and $a'$ cannot have the same value in their first register and $q_3$ cannot be covered. 
\end{example}
...
\begin{remark}
	...
	In this paper we will only study the "cover problem", but we argue that we can reduce from the query satisfaction problem to it:
	...
	This reduction is exponential, which does not hinder our complexity result as we are concerned with a complexity class stable by exponential reductions.
...
In fact, we may get rid of "local equality tests" at the cost of an exponential blow-up in the number of states.
...
The intuition is as follows: from a "protocol" $\prot$, we build a "protocol" $\prot'$ whose registers are used to store values of multiple registers of $\prot$ at the same time, so that equality of two registers of $\prot$ correspond to a condition on the states in $\prot'$. To do so, one must store in any state of $\prot'$ which register of $\prot$ is mapped to which register of $\prot'$, hence the exponential blowup. The full proof may be found in Appendix~\ref{sec:proof-lol-eq-test-elim}.
%\ifbasic

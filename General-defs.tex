...
%	 \lu{faire un parallèle avec l'exemple?}
...
\begin{remark}
	The reduction presented in the proof of Proposition~\ref{prop:reduction-LCS} can be adapted to show that the repeat coverability problem (asking whether there is a run in which some agent reaches a state $q_f$ infinitely many times) is undecidable for BNRA, as it is for LCS. This highlights the fact that the abstraction presented below relies on the fact that we only have to consider finite runs for coverability.
\end{remark}

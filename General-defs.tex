...
\begin{theorem}[Length function theorem \cite{SchmitzS2011upperHigman}]
	...
	Then there exists a function $f \in \Ffunction{\omega^{\size{\Sigma} - 1}}$ such that every "bad" sequence $w_1, w_2, \ldots$ such that $\size{w_i} \leq g^{(i)}(n)$ for all $i$ is of length at most $f(n)$. 
\end{theorem}

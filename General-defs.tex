...
A ""Broadcast Network of Register Automata"" (BNRA) \cite{DST2013} is defined by a finite transition system describing the behavior of an agent, which can broadcast and receive messages with integer values, store them in local registers, or make equality tests.
...
We use $r \geq 0$ to denote the number of registers in each node. 
%We consider a finite alphabet $\messages$ of messages.
...
\begin{definition}[Semantics]
	...
	A ""configuration"" over a set of agents $\agents$ is a labelling function $\config : \agents \to Q \times \nats^{\regnum}$, associating a state to each agent and a ""register value"" to each of its registers. 
...
\end{remark}
	...
	\AP The ""cover problem"" is to determine, given a "protocol" and a state $q$, whether there is a "run" of this protocol whose last "configuration" satisfies the "query" with the single formula $\quotemarks{q(\var{z})}$.
	
	\AP The ""target reachability problem"" is to determine, given a "protocol" and a state $q$, whether there is a "run" of this protocol whose last "configuration" $\config$ is such that $\st{\config}(\agentsof{\gamma}) = \{q\}$, i.e all agents reached $q$.

...
\begin{definition}
	...
	\AP A ""run"" is a sequence of steps $\run : \config_0 \step{} \config_1 \step{} \cdots \step{} \config_k$ such that $\config_0$ is an ""initial configuration"". We write $\config_0 \step{*} \config_k$ the existence of such a "run". Given a "run" $\run$, we write $\agentsof{\run}$ its set of agents.  

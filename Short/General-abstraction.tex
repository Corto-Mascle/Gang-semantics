...
\AP A ""reception step"" from $(q,\localdata)$ to $(q',\localdata')$ with transition $\atrans \in \transitions$ and value $\aval \in \nats$, denoted $(q,\localdata) \extbr{\atrans}{\aval} (q',\localdata')$, is defined when $\atrans$ is of the form $(q,\rec{m}{j}{\anact},q')$ with $\localdata(j') = \localdata'(j')$ for all $j' \neq j$ and:
	...
	For $\aval \in \nats$, the ""$\aval$-input"" $\intro*\vinput{\aval}{\localrun}$ (resp. ""$\aval$-output"" $\intro*\voutput{\aval}{\localrun}$) is the word $m_0 \cdots m_{\ell} \in \messages^*$ such that $(m_0, \aval) \cdots (m_{\ell}, \aval)$ is the projection of $\Input{\localrun}$ (resp. $\Output{\localrun}$) on $\messages \times \set{\aval}$.
...
\nico{j'ai completement enleve la preuve en annexe}
\end{restatable}
\begin{proof}
	Let $u := \localrunlabel{\node}$. For each "initial" value $v$ in $u$, $\node$ has at most $\size{\messages}$ "follower" children: if two "follower" children broadcast message type $m \in \messages$, then one could keep only the one that is able to broadcast it first.
	Because there are at most $r$ "initial" values, this makes at most $\size{\messages} \, r$ "follower" children and each one of them requires at most $K$ messages. If $\node$ is a "follower node", then the number of messages it must broadcast is bounded by $\size{\messages} \, r \, K +1$. If $\node$ is a "boss node", then its parent % if its exists, if root then empty spec
...
\cortoin{Je trouve cette preuve beaucoup trop vague.}

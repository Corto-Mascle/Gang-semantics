...
We explain this definition. Condition \ref{item:condition1_initial_value_sg} enforces that the "local run" cannot cheat by receiving its "initial values". 
 Condition \ref{item:condition3_boss_node_sg} expresses that $\localrunlabel{\node}$ broadcasts (at least) the messages of $\speclabel{\node}$. We can use the subword relation $\subword$, and not equality, because messages do not have to be received: \nico{terme lossiness jamais utilisé pour BNRA avant; unreliability of messages?} if some unnecessary messages are broadcast, we consider that they are not received.
Condition \ref{item:condition2_non_initial_value_sg} says that, for each value received, $\node$ has a child that broadcasts the right sequence of messages.

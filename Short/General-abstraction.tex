...
We describe what an agent needs from other agents as a set of \emph{"specifications@@sg"} which are words of $\messages^*$. 
...
\begin{figure}
	\resizebox*{!}{4.5cm}{
	\input{Figures/fig-illustration-signed-pumping}
	}
	\caption{Illustration of the proof of \cref{lem:towerbound_signature}: $s \in \transitions^{M}$ corresponds to the repeating sequence of transitions of length $M$. We make sure that we shorten the run so the new $\quotemarks{\enregact}$ actions are made on fresh values. If it was not the case on $u_2$ (here for example value $v$ is not fresh as it appeared before in the run), we introduce a new value never seen before (here value $v'$).}\label{fig:proof-pumping-signed}
\end{figure}
...
A key intuition for this proof is that if an agent that did not have $v$ initially broadcasts some message $(m,v)$, then intuitively we can isolate a subset of agents that did not have $v$ initially but that are able to broadcast $(m,v)$ after receiving a sequence of messages with that value. We can then copy them many times in the spirit of the "copycat principle". Each copy receives the necessary sequence of messages in parallel, and they then provide us with an unbounded supply of messages $(m,v)$. In short, if an agent is able to broadcast $(m,v)$ while not having $v$ as an "initial value", then we can consider that we have an unlimited supply of messages $(m,v)$.

...
\item $a_2$ broadcasts $\exgo$, $a_1$ receives, then $a_3$ broadcasts $\exgo$ and $a_1$ received: $\tuple{q_4,q_5,q_5}$.
\end{itemize}
Observe that the equality tests pass because $a_1$ stores $a_2$'s identifier in its second register and $a_3$'s identifier in its third register. 
% the following sequence is an initial run covering $q_4$:
	...
	\begin{figure}[t]
		...
		\vspace{-0.5cm}
		\caption{Example of an unfolding tree.}

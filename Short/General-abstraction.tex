...
We first prove decidability of \COVER for "signature BNRA". Note that, in "signature protocols", the initial values of "reception-only" registers are not relevant as they can never be shared with other agents. From this, we deduce the following informal observation:
\begin{observation}
\label{obs:sg_reception} 
When some agent $a$ receives a message, it can compare the value of the message only with the ones of previously received messages, \emph{i.e.}, check whether the sender is the same.
\end{observation}

Our aim is to understand what agent $a$ needs from other agents in order to perform some "local run" $u$ but in an actual "run". For this, $a$ needs to have its receptions matched with broadcasts. Because of Observation\ref{obs:sg_reception}, what matters is not the actual values of the receptions in $u$ but which ones are equal to which.  Therefore, for a given value $v$ received in $u$, if $m_1 \dots m_k \in \messages^*$ denotes the message types received in $u$ with value $v$, what $a$ needs is that some other agent $a'$ broadcasts all messages types from $m_1$ to $m_k$ in this order.   \nico{modif ici, a relire}
Therefore, we describe what an agent needs from other agents as a set of "specifications@@sg" which are words of $\messages^*$. 
...
To represent runs, we consider "unfolding trees@@signature" that represent such specifications, dependencies between them and how they are carried out. 
...
Let $\node$ a "node@@sg" of an "unfolding tree@@sg" $\tree$. Condition \ref{item:condition1_initial_value_sg} enforces that the "local run" cannot cheat by receving its "initial values" (an agent can never receive its "initial values" in a "run" of a "signature protocol"). 
 Condition \ref{item:condition3_boss_node_sg} expresses that $\localrunlabel{\node}$ broadcasts (at least) the messages of $\speclabel{\node}$. We can use the subword relation $\subword$, and not equality, because of the lossiness: some other messages may be broadcast, but we can assume that they are not received.
Condition \ref{item:condition2_non_initial_value_sg} expresses that for each value $v$ received $\node$ has a child that is able to broadcast the corresponding sequence of message types.
...
Let $\agents = \set{a_1, a_2,a_3}$, let $\run$ the run where the following occurs: we denote configuration a configuration $\config$ by $\tuple{\st{\config}(a_1),\st{\config}(a_2),\allowbreak \st{\config}(a_3)}$ (register values are omitted):
\begin{itemize}
...
% the following sequence is an initial run covering $q_4$:
	...
	Condition \ref{item:condition3_boss_node_sg} holds at every node because the local run of each node exactly broadcasts the "specification@@sg" of the node.\corto{shorten?} 
...
Given a "signature protocol" $\prot$ and a state $q_f$, $q_f$ is coverable in $\prot$ if and only if there exists an "unfolding tree@@sg" whose root is labelled by a "local run" $q_f$. We call such an "unfolding tree@@sg" a ""coverability witness@@sg"".
\end{lemma}
\begin{proof}
...
In the general case, a new phenomenon appears: an agent may broadcast a value that it did not have initially but that it received and stored. In particular, an agent starting with value $v$ could broadcast $v$ then require someone else to make a broadcast with value $v$ as well. For example, in the run described in \cref{ex:example-1}, the first agent needs to receive message $(m_4, 1)$ to reach $q_4$.
...
We obtain a computable bound (of the class $\Ffunction{\omega^\omega}$) on the "size@@tree" of a minimal "coverability witness", if it exists \nico{petit détail passable sous silence: une borne en $\Ffunction{\omega^\omega}$ ne donne pas la classe de complexite $\Fomegaomega$ (mais ça marche quand même car on obtient une borne en $\Ffunction{\alpha}$ avec $\alpha < \omega^\omega$)}. Our decidability procedure computes that bound, enumerates all trees of "size@@tree" below the bound and checks for each of them whether it is "coverability witness". Details can be found in Appendix~\ref{app:proofs_bounds} and Appendix~\ref{app:decidable_cover}.

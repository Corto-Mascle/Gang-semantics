...
\begin{figure}[t]
	...
	\vspace{-0.5cm}
	\caption{Example of an unfolding tree derived from $\run$. Grids correspond to "local runs", a row of a grid is a "local configuration". $\atrans_{jk}$ denotes the transition between state $q_j$ and state $q_k$, for example $\atrans_{01} = (q_0, \rec{\exready}{2}{\enregact}, q_1)$. Therefore, $\extlabel{\atrans_{jk}}{i}$ exists only if $\atrans$  Initial values of "reception-only" registers are irrevelant and written as $\quotemarks{\_}$. Colors of step labels correspond to message types.}

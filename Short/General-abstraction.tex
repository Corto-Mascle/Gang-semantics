\section{Cover Decidability for Signature Protocols}
...
We first prove decdability of \COVER for "signature BNRA". We start with the following observation: when an agent receives a message, it can compare the value of the message only with previously received values; the initial values in "reception-only" registers are not relevant as they can never be shared with other agents. Therefore, a given agent $a$ can only see which received messages have the same value, \emph{i.e.}, come from the same agent, so that what $a$ needs from other agents may be stated as a set of words where each word must be received in messages of the same value. 
...
An \emph{unfolding tree} is an abstraction of a "run" into a tree where each node is assigned a "local run" and the "specification" that it carries out. 
...
Given $\node$ a "node@@sg" of an "unfolding tree@@sg", condition \ref{item:condition1_non_initial_value_sg} expresses that the "specifications" of children of $\node$ are witnesses that messages received in the "local run" $\localrunlabel{\node}$ can be broadcast by other agents. Because the protocol is a "signature protocol", "initial" values of the "local run" should not be received. Condition \ref{item:condition2_boss_node_sg} expresses that $\node$ witnesses the broadcast of a sequence of messages $\speclabel{\node}$ with the same value. We can use the subword relation $\subword$, and not equality, because of the lossiness: it suffices to ensure that all received messages are broadcast, but some broadcast messages may not be received.  

\begin{example}
	Figure~\ref{fig:example-signature-protocol} provides an example of a "signature protocol".
	\begin{figure}[t]
		\label{fig:example-signature-protocol}
		\centering
		\input{Figures/fig-ex2}
		\caption{Example of a "signature protocol".}\label{fig:ex2}
	\end{figure}
Let $\agents = \set{a_1, a_2,a_3}$, let $\run$ the run where the following occurs, where $\tuple{q,q',q''}$ denotes the tuple of states of $a_1$, $a_2$ and $a_3$ (the values of the registers are omitted):
\begin{itemize}
...
\end{itemize}
Observe that the equality tests pass because $a_1$ stores $a_2$'s identifier in its second register and $a_3$'s identifier in its third register. Figure~\ref{fig:ex-unfolding-tree-signature} provides an unfolding tree retrieved from $\run$. A node of the tree corresponds to a given specification carried out in $\run$; because agents $a_2$ and $a_3$ broadcast to several other agents, they each correspond to several nodes of the tree.  
% the following sequence is an initial run covering $q_4$:
	...
	\begin{figure}[t]
		...
		\vspace{-0.5cm}
		\caption{Example of an unfolding tree derived from $\run$. Grids correspond to "local runs", a row representing a "local configuration"; transitions are omitted but can be retrieved from the protocol, for example the first step of $\localrunlabel{\node_1}$ is $q_0 \step{\rec{\exready}{2}{\enregact}} q_1$. Initial values of "reception-only" registers are irrevelant and written as $\quotemarks{\_}$.}
		\label{fig:unfolding-tree-sign}
	\end{figure}
	...
	We now explain why this tree is an "unfolding tree@@sg". Condition  \ref{item:condition1_non_initial_value_sg} is satisfied at $\node_1$, 
	for values $2$ and $3$, because $\node_2$ is a witness that $\exready \cdot \exgotwo$ can be broadcast and $\node_3$ is a witness that $\exready \cdot \exgothree$ can be broadcast.  
	It is also satisfied at $\node_2$, $\node_3$ and $\node_5$ because their local runs only receive $\exready$ and they each have a node with "specification@@sg" $\exready$. 
	It is trivially satisfied at $\node_4$ and $\node_6$ as their "local runs" have no reception. 
	Condition \ref{item:condition2_boss_node_sg} holds at every node because the local run of each node exactly broadcasts the "specification@@g" of the node. 
	...
	Observe that this tree can be shortened and still witness coverability of $q_4$. 
	Indeed, in the right branch, $\node_5$ and $\node_6$ share the same specification, therefore $\node5$ can be deleted and replaced with $\node_6$. In fact, we would have also been able to shorten the tree if we only had $speclabel{\node_5} \subword \speclabel{\node_6}$; we will use this argument to bound the size of "unfolding trees" that we have to consider.   
\end{example}
...
Hence, here, we cannot bound the "size@@tree" of the "unfolding tree" from the root to the leaves because of "follower nodes", nor from the leaves to the root because of "boss nodes". 

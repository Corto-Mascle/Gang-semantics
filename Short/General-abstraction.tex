...
% the following sequence is an initial run covering $q_4$:
	...
	Condition \ref{item:condition3_boss_node_sg} holds at every node because the local run of each node exactly broadcasts the "specification@@sg" of the node. 
...
Given a "signature protocol" $\prot$ with a state $q_f$, $(\prot,q_f)$ is a positive instance of \COVER if and only if there exists an "unfolding tree@@sg" "covering@ut" $q_f$. We call such an "unfolding tree@@sg" a ""coverability witness@@sg"".
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof sketch]
	% As in Lemma~\ref{lem:coverability_witness_sg}, we translate an "unfolding tree" into a "run" and \emph{vice-versa}. 
%	\nico{peut etre reduit plus en enlevant des details techniques, voire completement enlevé}
...
\begin{restatable}{lemma}{lemBoundSuccessorHeight}
	...
	There exists $B \in \nats$ s. t. for all $K \in \nats$ and $\node$ in $\tree$, if all neighbors of $\node$ of higher altitude have size at most $K$ then $\size{\node} \leq B \towerfun(\size{\prot}, r) \, (\size{\messages} \, r \, K +1)$. 
\end{restatable}
\begin{proof}

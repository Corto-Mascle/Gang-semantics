...
\begin{example}\label{ex:example-1}
	...
	\end{multline*}
	Here, the broadcast messages are, in this order: $(m_2,1)$ by $a_1$, $(m_4,4)$ by $a_2$, $(m_6,1)$ by $a_1$ and $(m_7,1)$ by $a_1$ and they are all received by the other agent. 
\end{example}
...
\begin{remark}
	In \cite{DelzannoST13}, the authors consider the \AP \emph{query problem} where one looks for an "initial run" reaching a "configuration" satisfying some \emph{queries}.	
	In fact, one can exponentially reduce this problem to \COVER hence our complexity result of $\Fcomplexity{\omega^\omega}$ also holds for the query problem. In the case with one register, one can even find a polynomial-time reduction hence our \NP result also holds with queries. 

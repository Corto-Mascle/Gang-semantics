...
\AP We finally introduce \intro{signature BNRA}, an interesting restriction of our model where register $1$ is ""broadcast-only"" and all other registers are ""reception-only"", an example of such a protocol is displayed in \cref{fig:example-signature-protocol}. Said otherwise, the first register acts as a permanent identifier with which agents sign their messages. Under this restriction, a message is composed of a message type along with the identifier of the sender. This restriction is relevant for pedagogical purposes: we will see that it falls into the same complexity class as the general case but makes the decidability procedure simpler. 

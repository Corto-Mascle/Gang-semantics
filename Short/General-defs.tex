...
\begin{definition}[Semantics]
	...
	\nico{modif pour préciser l'ensemble d'agents}
...
\begin{definition}
	...
	\AP The ""coverability problem"" \COVER asks, given a "protocol" $\prot$ and a state $q_f$, whether there is a finite non-empty set of agents $\agents$, an "initial run" $\config_0 \step{*} \config_f$ over $\agents$ that ""covers"" $q_f$, \emph{i.e.}, there is $a \in \agents$  such that $\st{\config_f}(a)  = q_f$.
	
	\AP The ""target problem"" \TARGET asks, given a "protocol" $\prot$ and a state $q_f$, whether there is there is a finite non-empty set of agents $\agents$ and an "initial run" $\config_0 \step{*} \config_f$ over $\agents$  such that, for every $a \in \agents$, $\st{\config_f}(a) = q_f$, \emph{i.e.}, all agents end on $q_f$.
\end{definition}
...
Each agent has at most one predecessor, so the communication graph is a forest where messages propagate from roots to leaves.
 ...
 The full proof can be found in Appendix~\ref{app:reduction-lcs}. \nico{modif, parle maintenant de forets de branches (modifs dans l'annexe égalment)}
\end{proof}

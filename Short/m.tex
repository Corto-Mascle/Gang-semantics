...
\begin{document}
\title{Parameterized Broadcast Networks with Registers: from NP to the Frontiers of Decidability\thanks{\footnotesize{Partly supported by ANR PAVEDYS.}}}
...
%\input{Figures/zoom-on-tree}
	...
	One may want to enrich our protocols
	% with a stack. While this adds little difficulty in the general case (it suffices to extend Lemma~\ref{lem:short-local-runs} to pushdown protocols using a classical cut-hill argument), the case of one register may be trickier.
	by the addition of inequality tests, as done in classical models such as data nets  \cite{datanetsinequalityfomegaomegaomega}. 

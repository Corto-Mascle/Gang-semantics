...
\begin{definition}
	...
	A \intro{gang} is a pair $\gang = (\boss, \clique) \in (Q \cup \set{\noboss}) \times \powerset{Q}$. The element $\boss$ is the ""boss@gang"" and the set $\clique$ is the \reintro{clique} of the "gang". 
...
\begin{definition}
	...
	Given a set of states $S \subseteq Q$, a "message type" $m$ and a set of "actions" $A$, we define $\cliquesucc{S}{m}{A} = \set{s' \in Q \mid \exists s \in S, a\in A, (s, \recone{m}{a}, s') \in \Delta}$.

\begin{tikzpicture}[xscale=0.5,AUT style,node distance=2.1cm,auto,>= triangle
	...
	\node[state] [right of=0, yshift=20, xshift=-10] (x) {};
	\node[state] [right of=0, yshift=-20, xshift=-10] (y) {};
	\node[state] [right of=x, xshift=20] (1) {$q_1$};
	\node[state] [right of=y, xshift=20] (2) {$q_2$};
	
	\draw[color=white] (-2,-2.5) rectangle (15,3);
	
;	\path[->, bend left=10] 	
	(0) edge node[above, xshift=-10] {$\br{init}{1}$} (x)
	;
	\path[->, bend right=10] 	
	(0) edge node[below, xshift=-15] {$\rec{init}{2}{\enregact}$} (y)
	;
	\path[->] 	

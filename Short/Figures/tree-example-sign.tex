...
\newcommand{\displayone}[1]{{\color{Yellow!50!black} #1}}
\newcommand{\displaytwo}[1]{{\color{cyan} #1}}
\newcommand{\displaythree}[1]{{\color{Red} #1}}
\begin{tikzpicture} [xscale = 0.94, yscale = 0.6, every node/.style = {scale = 0.9}]
	...
	\node [align=center, font = {\footnotesize}] (sp) at (-3.2, -3-0.8) {Node $\node_2$ ($a_2$)};
	\node [align = center, font = {\footnotesize}] (sp) at (-3.2,-3.5-0.8) {$\spec = \displayone{\exready} \cdot \displaytwo{\exgotwo}$};
	\node [align = center, font = {\footnotesize}] (sp) at (-3.2, -4-0.8) {$v = 2$};
	\end{scope}
	\begin{scope}[xshift = 5]
	% \regbar{3}{-3}

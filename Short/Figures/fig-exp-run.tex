\begin{tikzpicture}[xscale=0.5,AUT style,node distance=2.1cm,auto,>= triangle45]
	...
	;
	\path[->]
	(0a) edge node[right] {$\recone{m_0}{\enregact}$} (0b)
	(0b) edge node[right] {$\brone{m'_0}$} (0c)
	
	(1a) edge node[right] {$\recone{m_1}{\enregact}$} (1b)
	(1b) edge node[right] {$\brone{m'_1}$} (1c)
	
	(Na) edge node[right] {$\recone{m_N}{\enregact}$} (Nb)
	(Nb) edge node[right] {$\brone{m'_N}$} (Nc)
	
	(0a) edge node {$\brone{m_0}$} (0')
	(0') edge node {$\recone{m'_0}{\eqtestact}$} (1a)
	
	(Na) edge node {$\brone{m_N}$} (N')
	(N') edge node {$\recone{m'_N}{\eqtestact}$} (end)

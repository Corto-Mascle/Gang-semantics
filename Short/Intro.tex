...
% Initially, agents have distinct values in their registers. 
...
This model was introduced in \cite{DelzannoST13}, as a natural extension of Reconfigurable Broadcast Networks~\cite{DelzannoSZ2010Adhoc}. This paper established that the problem is undecidable if the agents are allowed to send two values per message. It moreover claimed that, in our model with one value per message, the "coverability problem" was decidable and even \PSPACE-complete; however, the proof turned out to be wrong \cite{ArnaudErratum}. As we will see, the complexity of that problem is in fact much higher.

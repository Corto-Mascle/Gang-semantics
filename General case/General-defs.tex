...
\begin{definition}[Syntax]
	...
	Transitions labelled with $\brsymb$ are ""broadcasts"", transitions labelled with $\recsymb$ are ""receptions"" and transitions labelled with $\locsymb$ are ""local transitions"".
	Given a reception $\rec{\amessage}{i}{\anact}$ or a local transition $\loc{i}{j}{\anact}$, $\anact$ is the ""action"" of the reception (resp. local transition).
	
	A ""simple protocol"" is one that does not have any local transitions or receptions with $\dummyact$.
...
\begin{definition}[Semantics]
	...
	\begin{itemize}
		\item there exists $a \in \agents$ and a "local transition" $(\st{\config}(a), \loc{j_1}{j_2}{\anact}, \st{\config'}(a))$ such that for all $a'\neq a$, $\config(a') = \config'(a')$ and
		\begin{itemize}

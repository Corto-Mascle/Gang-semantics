...
\begin{definition}
	...
	Simiarly, its ""output"" is the sequences $\Output{\localrun}$ of messages broadcasted in $\localrun$.
	...
	Simiarly, its ""$v$-output"" $\voutput{v}{u}$ is the sequence of messages with value $v$ broadcasted in $\localrun$.
	Its $v$-vision, which we denote by $\vision{v}{\localrun}$, is the sequence of messages in $\localrun$ with value $v$.
...
\begin{lemma}
	...
	If $\mu$ is an ancestor of $\mu'$, $\decsymb' \subword \decsymb$ and $m=m'$ then there exists a smaller "tree unfolding" with its root labelled $w$.  
\end{lemma}

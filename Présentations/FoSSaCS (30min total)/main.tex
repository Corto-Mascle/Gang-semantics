...
\author[Nicolas Waldburger]{\begin{tabular}{c} Nicolas Waldburger \textit{\scriptsize(IRISA, Univ. Rennes)} \\
	...
	\addtobeamertemplate{title page}{}{\begin{center}{FoSSaCS'24}\end{center}}
%	
%	
\begin{frame} 	
	\titlepage
...
\begin{frame}{Bounding the tree}
	...
	Using some (involved) shortening arguments, we obtain a bound on the length of local runs: 
...
\begin{frame}{Bounding the branches}
	...
	\onslide<3->{
	Length function theorem: we obtain a computable bound $B(|\mathcal{P}|)$ such that $n \leq B(|\mathcal{P}|)$: $B$ bounds the height of a witness tree for \COVER !
	}
\end{frame}
...
\begin{frame}{Decidability}
	...
	\begin{block}{Theorem}
		The {\COVER} problem for \only<2-3>{\color{blue}signature\color{black}}\only<4->{\color{blue}\st{signature}\ \color{black}} BNRA is decidable \onslide<3->{\color{red!70!black} and in $\mathbf{F}_{\omega^\omega}$.}
	\end{block} 
	\onslide<3->
	The length function theorem bound is in hyper-Ackermannian class $\mathcal{F}_{\omega^\omega}$.
	
	\onslide<4->
	This can be extended to the general case, where processes may send back values that they have received and stored. This requires more general witness trees, and more work. 
...
\begin{frame}{Complexity results}
	...
	\footnotetext<1->{Schnoebelen, Information Processing Letters '08}
\end{frame}

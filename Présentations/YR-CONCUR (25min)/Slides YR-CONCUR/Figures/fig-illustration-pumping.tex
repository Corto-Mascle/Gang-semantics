...
%		\draw (5.4,-3) -- (5.4,3);
		...
		\node (txt) at (11,3.2) {Say we can reduce any local run with $<n$ active registers of length $\geq M$.};
		}
		...
		\node (r1) at (5,1.75) {$\reg_1 $};
		\node (r2) at (5,1.25) {$\reg_2 $};
		\node (r3) at (5,0.75) {$\reg_3 $};
		\node (r4) at (5,0.25) {$\reg_4 $};
	}
...
%			\node (s) at (9.9, 2.4) {$q''$};
		...
		\node (d) at (13.25,-0.5) {$M$};
		
		\draw[<->] (7, -0.2) -- (9.5,-0.2);
		\node (d) at (8.25,-0.5) {$M$};
		
		\draw[<->] (6, -0.7) -- (15,-0.7);
		\node (d) at (10.5,-1) {$> |\Delta|^M$};
		}
